...
read -r TOOL_COUNT TOOL_NAMES WITH_DESC WITH_SCHEMA <<EOF
...
print(len(tools), names, with_desc, with_schema)
PY
)
EOF
log "tools retornadas: count=$TOOL_COUNT names=[$TOOL_NAMES] com_descricao=$WITH_DESC com_inputSchema=$WITH_SCHEMA"
...
[[ "$TOOL_COUNT" == "${ANNOUNCED:-x}" ]]            || { ok="no"; log "  ! contagem difere do anúncio de startup"; }
