...
| Workflows servidos | `tests/cloud` → `DocPipeline`, `SlowBuild` |
...
`run.sh` autocontido (copia `tests/cloud/mhl` para a própria pasta se não existir;
...
- `../Dockerfile` (+ `../Dockerfile.dockerignore`) — build multi-stage, cross-compila para a
  ...
  `docker build -f tests/cloud/Dockerfile -t mhl-serve:local .` (a partir da raiz do repo).
