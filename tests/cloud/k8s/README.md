...
```sh
kubectl apply -f tests/cloud/k8s/namespace.yaml
kubectl -n mhl-serve create secret generic mhl-serve-token \
  --from-literal=token="$(openssl rand -hex 24)" \
  --dry-run=client -o yaml | kubectl apply -f -

kubectl apply -k tests/cloud/k8s          # service + deployment (skips secret.yaml if you kustomize-exclude it)
# or, plainly:
kubectl apply -f tests/cloud/k8s/service.yaml -f tests/cloud/k8s/deployment.yaml
...
kubectl apply -f tests/cloud/k8s/pvc.yaml -f tests/cloud/k8s/deployment-durable.yaml
kubectl -n mhl-serve rollout status deploy/mhl-serve --timeout=120s
