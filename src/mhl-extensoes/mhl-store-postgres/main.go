...
// and compare_and_swap, single atomic statements the `mhl serve` cross-replica
// run lock is built on.
//
//	extension store S {
...
func (s *store) handleCall(msg rpc) rpc {
	...
	case "put_if_absent":
		...
		acquired, err := s.pg.putIfAbsent(ctx, key, b)
		if err != nil {
			res = fail(err.Error())
		} else {
	...
	case "compare_and_swap":
		...
		swapped, err := s.pg.compareAndSwap(ctx, key, exp, b)
		if err != nil {
			res = fail(err.Error())
		} else {
