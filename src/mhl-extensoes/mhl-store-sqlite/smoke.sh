#!/usr/bin/env bash
# smoke.sh — end-to-end check for the mhl-store-sqlite extension.
#
#   make smoke          # builds bin/ and runs this
#   ./smoke.sh          # assumes `make build` already ran
#
# No external service needed — SQLite is a local file. Override the runtime
# with:  MHL=/path/to/mhl ./smoke.sh
set -euo pipefail
...
[[ -x "$HERE/bin/mhl-store-sqlite" ]] || { echo "bin/mhl-store-sqlite missing — run: make build" >&2; exit 1; }

PROJ="$(mktemp -d "${TMPDIR:-/tmp}/mhl-store-sqlite-smoke.XXXXXX")"
PROBE="$PROJ/wire.jsonl"
trap 'rm -rf "$PROJ"' EXIT

cat > "$PROJ/main.mh" <<EOF
extension store S {
    path: "$PROJ/state/mhl.db"
    table: "mhl_store"
    log: "$PROBE"
}

pipeline SqliteRoundTrip {
    step seed {
...
[[ -f "$PROJ/state/mhl.db" ]] || fails+=("the database file was never created")

if [[ ${#fails[@]} -eq 0 ]]; then
  echo "PASS — get/put/delete/list + cas round-trip through SQLite"
else
  printf 'FAIL:\n'; printf '  - %s\n' "${fails[@]}"
  exit 1
fi
