...
//	install <dir>  — vendor an extension into .mhl/extensions/ and pin it
//
// Downloading an extension is deliberately not offered: there is no
// download-and-run without a trust policy (see docs/plano-extensoes-mcp-a2a.md).
...
func extensionInstall(args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: mhl extension install <extension-dir>")
	}
	src := args[0]
	...
	lock.Extensions[m.ID] = external.LockEntry{Version: m.Version, SHA256: sum}
