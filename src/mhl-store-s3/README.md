...
An **official** `store`-kind [mhl extension](../../sample/extensions/extension-protocol.md)
backed by **Amazon S3** — or any S3-compatible object store (MinIO, Cloudflare
R2, Ceph RGW).

It is a drop-in replacement for the reference
[`store-fs`](../../sample/extensions/store-fs/): the same wire contract (kind
...
```sh
cd src/mhl-store-s3
make build      # -> bin/mhl-store-s3  (ad-hoc codesigned on macOS)
...
mhl extension install /path/to/src/mhl-store-s3
