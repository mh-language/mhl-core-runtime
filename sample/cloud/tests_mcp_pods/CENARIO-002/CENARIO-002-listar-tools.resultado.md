...
| Comando do servidor | `mhl serve mcp --http --addr 127.0.0.1:8712 --token <gerado> --state-dir <tmp> sample/cloud` |
| Endpoint | `POST http://127.0.0.1:8712/mcp` |
| Script | [`run.sh`](run.sh) |

## Passos executados

1. **Dado que o servidor MCP está em execução** — script sobe `mhl serve mcp --http` e aguarda `GET /healthz` = `200`. Startup anuncia `2 tool(s) from .../sample/cloud`.
