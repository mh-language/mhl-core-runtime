...
#
# Constroi a imagem mhl-serve:local, aplica sample/cloud/k8s/, e verifica:
...
K8S_DIR="$REPO_ROOT/sample/cloud/k8s"
NS="mhl-serve"
IMG="mhl-serve:local"
LPORT="8791"
...
if [[ "${SKIP_BUILD:-0}" == "1" ]]; then
  ...
  log "docker build -f sample/cloud/Dockerfile -t $IMG . (a partir de $REPO_ROOT)"
  ( cd "$REPO_ROOT" && docker build -f sample/cloud/Dockerfile -t "$IMG" . ) >"$L/docker-build.log" 2>&1 \
