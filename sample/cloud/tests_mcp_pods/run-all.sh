#!/usr/bin/env bash
# run-all.sh — regressivo de todos os cenários de sample/cloud/tests_mcp_pods.
#
...
REF_MHL="$HERE/../mhl"                 # sample/cloud/mhl — binário de referência
RUN_K8S="${RUN_K8S:-0}"
VERBOSE="${VERBOSE:-0}"
TIMEOUT="${TIMEOUT:-300}"
RESIGN="${RESIGN:-1}"
...
if [[ ! -x "$REF_MHL" ]]; then
  ...
  log "        rode: make -C src/mhl-runtime build && cp src/mhl-runtime/dist/mhl sample/cloud/mhl"
  exit 2
fi
