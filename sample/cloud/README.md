...
- **`extension store S { dir: ... }`** in a `.mh` file under the serve dir —
  binds a `store`-kind extension (`sample/extensions/store-fs` is the reference;
...
```bash
STATE=$(mktemp -d)
"$MHL" serve mcp --http --addr 127.0.0.1:8711 --token "$MHL_TEST_TOKEN" --state-dir "$STATE" sample/cloud &
