...
**Objetivo:** Verificar que a extensão **oficial** `src/mhl-store-s3` (kind
...
```gherkin
Dado um MinIO local (docker compose de src/mhl-store-s3) e o bucket mhl-state
...
- SigV4 é assinado pela própria extensão (sem dependências); o teste unitário
  `src/mhl-store-s3/s3_test.go` fixa a assinatura no vetor documentado da AWS.
