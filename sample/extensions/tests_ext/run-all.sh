#!/usr/bin/env bash
# run-all.sh — regressivo de todos os cenários de sample/extensions/tests_ext.
#
...
#
# Cenários S3 (011+) sobem um MinIO via `docker compose` de src/mhl-store-s3/ e
# se auto-PULAM (SKIP, não FAIL) quando o Docker não está disponível. Ao final o
# MinIO é derrubado (`down -v`); passe S3_KEEP=1 para mantê-lo de pé.
...
[[ -x "$ROOT/mhl" ]] || { log "${c_red}FALHA${c_rst}: $ROOT/mhl ausente — rode: cp sample/cloud/mhl sample/extensions/mhl"; exit 2; }
...
# derruba o MinIO dos cenários S3 (a menos que S3_KEEP=1)
S3_COMPOSE="$ROOT/../../src/mhl-store-s3/docker-compose.yml"
if [[ "${S3_KEEP:-0}" != "1" && -f "$S3_COMPOSE" ]] && command -v docker >/dev/null 2>&1; then
  docker compose -f "$S3_COMPOSE" down -v >/dev/null 2>&1 || true
  log "MinIO (src/mhl-store-s3) derrubado — S3_KEEP=1 para manter"
fi
