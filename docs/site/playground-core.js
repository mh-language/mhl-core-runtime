(function (root) {
  "use strict";
  const keywords = new Set("agent router prompt pipeline workflow loop partial entry parallel step input output memory mem tool extension extensible test describe var const enum type if else match while for in return break import from export as true false null max spawn wait any of timeout try catch finally goto".split(" "));
