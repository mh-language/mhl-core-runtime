#!/usr/bin/env bash
# CENARIO-016 — mhl serve mcp --http com `extension store S` -> PostgreSQL
# Análogo do CENARIO-013 (S3) e do CENARIO-010 (store-probe + FS local).
...
ADDR="127.0.0.1:8792"; BASE="http://$ADDR"; TOKEN="cenario-ext-016-$(date +%s)"
...
state_keys() { # <outfile>  — todas as chaves lógicas na tabela
  pg_psql "SELECT key FROM mhl_store ORDER BY key" 2>/dev/null | sed '/^$/d' | sort > "$1"
}

########################################################################
# Parte A — estado durável no Postgres + resume + restart-reclaim
########################################################################
...
log "chaves no store (Postgres):"; cat "$L/state-keys.txt" | tee -a "$CLIENT_LOG"
...
stop_server "$SRV"; SRV=""
log "servidor 1 encerrado — subindo servidor 2 no mesmo banco"
start_server --max-concurrent-runs 2
SRV2="$SRV"; SRV=""
...
log "store no fim (Postgres):"; cat "$L/state-keys-final.txt" | tee -a "$CLIENT_LOG"
...
grep -q "^session/" "$L/state-keys.txt"                    || fails+=("store (Postgres) sem session/")
grep -Eq "^run/${RID}/" "$L/state-keys.txt"                || fails+=("store (Postgres) sem run/$RID/ (checkpoint per_step)")
...
if [[ ${#fails[@]} -eq 0 ]]; then
  ok "estado durável de mhl serve no Postgres (session/* + run/<id>/*); resume ok; reclaim pós-restart da tabela; runs concorrentes com chaves disjuntas"
else
