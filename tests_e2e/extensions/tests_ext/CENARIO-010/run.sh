...
ensure_env
new_project
STATE="$PROJ/state"; PROBE_LOG="$PROJ/probe.jsonl"; mkdir -p "$STATE"
CLOUD="$(cd "$ROOT/../cloud" && pwd)"
cp "$CLOUD/docs-workflow.mh" "$CLOUD/slow-build.mh" "$PROJ/" || die "faltam docs-workflow.mh/slow-build.mh em $CLOUD"
...
ADDR="127.0.0.1:8781"; BASE="http://$ADDR"; TOKEN="cenario-ext-010-$(date +%s)"
...
tree_of() { find "$STATE" -type f | sed "s|$STATE/||;s|\\.json\$||" | sort; }
...
tree_of > "$L/state-tree.txt"
log "store da extensão:"; cat "$L/state-tree.txt" | tee -a "$CLIENT_LOG"
...
########################################################################
# Parte B — duas runs concorrentes (approved=no → param no gate) têm chaves
# run/<id>/checkpoint disjuntas na extensão
########################################################################
...
tree_of > "$L/state-tree-final.txt"
log "store no fim:"; cat "$L/state-tree-final.txt" | tee -a "$CLIENT_LOG"
# run ids distintos com checkpoint vivo no store agora (RID2 parado + CR1 + CR2)
RUN_IDS_IN_STORE=$(grep -Eo '^run/[a-f0-9]{16,}/' "$L/state-tree-final.txt" | sed 's|^run/||;s|/$||' | sort -u)
...
grep -q "^session/" "$L/state-tree.txt"                    || fails+=("store sem session/")
grep -Eq "^run/${RID}/" "$L/state-tree.txt"                || fails+=("store sem run/$RID/ (checkpoint per_step)")
...
if [[ ${#fails[@]} -eq 0 ]]; then
  ok "run/*+session/* na extensão; resume ok; reclaim pós-restart do store; múltiplas runs com chaves disjuntas"
else
