...
read -r COUNT NAMES WITH_DESC WITH_SCHEMA RESULT_TYPE SRV_INFO WF_COUNT CTL_COUNT <<EOF
...
# The HTTP transport also lists the synthetic mhl_run_* control tools.
ctl = sum(1 for t in tools if t.get("name","").startswith("mhl_run_"))
wf = len(tools) - ctl
print(len(tools), names, wd, ws, rt, si, wf, ctl)
PY
)
EOF
log "stateless tools/list: count=$COUNT (workflows=$WF_COUNT, control=$CTL_COUNT) names=[$NAMES] com_descricao=$WITH_DESC com_inputSchema=$WITH_SCHEMA resultType=$RESULT_TYPE serverInfo=$SRV_INFO"
...
[[ "$WF_COUNT" == "${ANNOUNCED:-x}" ]]           || { ok="no"; log "  ! contagem de workflows difere do startup"; }
...
echo "$NAMES" | grep -q "mhl_run_start"          || { ok="no"; log "  ! mhl_run_start ausente"; }
