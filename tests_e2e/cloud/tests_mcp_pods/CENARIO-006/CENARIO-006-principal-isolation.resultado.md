...
| Item | Valor |
|---|---|
| Binário | `tests/cloud/tests/CENARIO-006/mhl` (cópia de `tests/cloud/mhl`) |
| Versão | `mhl v1.1.0-alpha-7-g418d7dc-dirty` |
| Servidor | `mhl serve mcp --http --addr 127.0.0.1:8724 --token <gerado> --principal-header X-Mhl-Principal --state-dir <tmp> tests/cloud` |
...
- `context.principal` chegando ao workflow não foi verificado (exige um bloco `context:`
  no `.mh`, que os workflows de `tests/cloud` não têm).
