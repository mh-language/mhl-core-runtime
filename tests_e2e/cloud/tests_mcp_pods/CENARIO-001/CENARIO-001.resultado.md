...
| Item | Valor |
|---|---|
| Binário | `tests/cloud/tests/CENARIO-001/mhl` (cópia de `src/mhl-runtime/dist/mhl`) |
| Versão | `mhl v1.1.0-alpha-6-g439ea52` |
| Comando do servidor | `mhl serve mcp --http --addr 127.0.0.1:8711 --token <gerado> --state-dir <tmp> tests/cloud` |
| Endpoint | `POST http://127.0.0.1:8711/mcp` |
| Workflows carregados | 2 tools (`DocPipeline`, `SlowBuild`) de `tests/cloud` |
...
- [x] Log do servidor MCP mostrando o endpoint no ar e as tools carregadas — [`logs/mcp-server.log`](logs/mcp-server.log)
  (`mhl serve mcp --http: 2 tool(s) from .../tests/cloud on http://127.0.0.1:8711/mcp`)
